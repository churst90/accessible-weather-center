...
- **The test-count backstop failed every CI run, on a green suite.** It matched junit's `file="…"` attribute against absolute bundle paths; that attribute is not stable across Node versions, so on CI's Node 20 every file scored zero — indistinguishable, to the check, from a run where nothing executed. All 336 tests passed and the build failed claiming none had run. Matched on basename now, with a total-count fallback if a future Node drops the attribute entirely, because failing open would defeat the point of the check. This had never surfaced before for the reason above: `npm ci` was broken, so CI had never once reached `npm test`.
